...
@Sharable
public class SystemEnvironmentVariablesRequestHandler extends HttpRequestUriChannelHandler {

	public SystemEnvironmentVariablesRequestHandler(String uri) {
		super(uri);
	}
	...
	protected void handleRequest(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
		if(Objects.equals(HttpMethod.GET, request.method())){
			handleGetEnvironmentVariables(ctx, request);
		}
	}
	
	private void handleGetEnvironmentVariables(ChannelHandlerContext ctx, FullHttpRequest request){
