...
public class CoapThyngClient implements ThyngClient {
	...
	@Override
	public GatewayDetailsDTO registerAndFetchDetails() throws Exception{
	...
	@Override
	public void sendTelemetry(TelemetryRequest telemetryRequest) throws Exception{
		final ByteArrayOutputStream baos = new ByteArrayOutputStream();
		final DataOutput out = new DataOutputStream(baos);
		final byte dataType = telemetryRequest.getTelemetry().write(out);
		final Long gatewayId = properties.getLong(Constant.KEY_GATEWAY_ID, null);
		...
		request.getOptions().setUriQuery("gatewayId="+gatewayId+"&thingId="+telemetryRequest.getThingId()
		+"&sensorId="+telemetryRequest.getSensorId()+"&dataType="+dataType);
		Optional.ofNullable(client.advanced(request)).ifPresent(response -> {
			Optional.ofNullable(telemetryRequest.getAfter()).ifPresent(after -> after.run());
			if(response.isSuccess()){
				Optional.ofNullable(telemetryRequest.getSuccess()).ifPresent(success -> success.run());
			}
		});
	}
